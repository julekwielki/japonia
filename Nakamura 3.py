...
print(perr, np.exp(popt[0])*perr[0])
...
ax[1].plot(x1_scaled, [function_lin(x, popt[0], popt[1]) for x in x1_log], 'g--', label='fit: a=%.5f, n=%.3f' % (np.exp(popt[0]), popt[1]))
# ax[0].scatter(x1_log, y1_log)
ax[1].errorbar(x1_scaled, y1_log, yerr=sd1_log, fmt='o', label="data")
# ax[1].legend()
ax[1].set_xscale('log')
ax[1].set_xlabel("time")
ax[1].set_ylabel("linearised")
...
print(perr2, np.exp(popt2[0])*perr2[0])

fig, ax = plt.subplots(2, 1, figsize=(8, 8))

ax[0].plot(xx2, [function(x, np.exp(popt2[0]), popt2[1]) for x in xx22], 'g--', label='fit: a=%.5f, n=%.3f' % (np.exp(popt2[0]), popt2[1]))
ax[0].errorbar(xx2, data_can2, yerr=sd2, fmt='o', label="data")
#ax[0].legend()
ax[0].set_xlabel("time (weeks)")
ax[0].set_ylabel("carcinoma probability genotype 1  - BRCA1 L63X/+")

ax[1].plot(xx22, [function_lin(x, popt2[0], popt2[1]) for x in x2_log], 'g--', label='fit: a=%.5f, n=%.3f' % (np.exp(popt2[0]), popt2[1]))
# ax[0].scatter(x1_log, y1_log)
ax[1].errorbar(xx22, y2_log, yerr=sd2_log, fmt='o', label="data")
# ax[1].legend()
ax[1].set_xscale('log')
ax[1].set_xlabel("time")
ax[1].set_ylabel("linearised")
